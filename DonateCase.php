...
class DonateCase extends PluginBase implements Listener{
	...
	public function onInteract(PlayerInteractEvent $e) {
		...
		if($b->x == $coord['x'] && $b->y == $coord['y'] && $b->z == $coord['z']) {
			$e->setCancelled(true);
			if(!$this->check) {
				$p->sendMessage('§5[§cДонат-§6кейс§5] §fКейс занят! Дождитесь своей очереди!');
				return true;
			}
			if(empty($this->list->get(strtolower($p->getName())))) {
				$p->sendMessage('§5[§cДонат-§6кейс§5] §fУ вас нет кейсов для открытия!');
				return true;
			}
			$this->check = false;
			...
			shuffle($color);
			$wins = ['Guest','Owner','Guest','Admin','Owner','Admin','OP'];
			shuffle($wins);
			$this->win = $wins;
			...
			$this->wins = $wins;
			//$sender->getLevel()->addParticle(new FloatingTextParticle(new Vector3($b->x, $b->y, $b->z), '', $this->wins));
	...
	public function createFloatingText($text, $eid, $x, $y, $z, $level = false, $player = false, $b, $one) {
		...
		$this->getScheduler()->scheduleDelayedTask(new Rotat($this, $pk, $player, 0, $b, $x, $y, $z, $one), 1);
	}
}
...
class Rotat extends PluginTask{
	...
	public function onRun($tick){
		if($this->i == 30) {
			...
			$this->player->sendMessage('§5[§cДонат-§6кейс§5] §fВы выиграли '.$this->plugin->wins[1].'§f. Спасибо за игру!');
		...
		if($this->plugin->stop) {
			$this->plugin->getScheduler()->scheduleDelayedTask(new Delete($this->plugin, $this->pk, $this->player), 20 * mt_rand(2.1,2.9));
			return true;
		}
		if($this->one) {
		...
		$this->plugin->getScheduler()->scheduleDelayedTask(new Rotat($this->plugin, $pk, $this->player, $this->i, $this->b, $this->x, $this->y, $this->z, $this->one), 1);
	}
}
